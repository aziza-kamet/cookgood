 ...
 * @author Aziza
 */
@WebServlet(name = "ConfirmationServlet", urlPatterns = {"/email_confirmation"})
public class ConfirmationServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String hash = null;
            if((hash = request.getParameter("hash")) != null){
                
                DBConnection db = (DBConnection) request.getServletContext().getAttribute("dbBean");
                UserBean user = null;
                if((user = db.confirmRegistration(hash)) != null){
                    request.setAttribute("user", user);
                    RequestDispatcher disp = request.getRequestDispatcher("auth");
